...
static bool add_overlay(private_dumm_t *this, char *dir)
{
	...
	{
		DBG1("overlay directory string '%s' is too long", dir);
		return FALSE;
	}
	if (access(dir, F_OK) != 0)
	{
		...
		{
			DBG1("creating overlay directory '%s' failed: %m", dir);
			return FALSE;
		}
	}
	...
	while (enumerator->enumerate(enumerator, (void**)&guest))
	{
		...
		size_t len = snprintf(guest_dir, sizeof(guest_dir), "%s/%s", dir,
		...
		if (access(guest_dir, F_OK) != 0)
		{
			...
			{
				DBG1("creating overlay directory for guest '%s' failed: %m",
					 guest->get_name(guest));
				goto error;
			}
		}
...
static bool load_template(private_dumm_t *this, char *name)
{
	...
	{
		DBG1("template name '%s' is too long", name);
		return FALSE;
	}
	if (strchr(name, '/') != NULL)
	{
		DBG1("template name '%s' must not contain '/' characters", name);
		return FALSE;
	}
